...
out_model_file = os.path.join(outfolder, 'encoder'+ outsuffix +'.h5')
encoder.save(out_model_file)

out_model_file = os.path.join(outfolder, 'decoder'+ outsuffix +'.h5')
decoder.save(out_model_file)
...
sns.distplot(encoder_weights, hist=True, kde=True, 
             ...
             kde_kws={'linewidth': 4})
# Add labels
plt.suptitle('Dist. encoder weights')
plt.xlabel('Weights')
plt.ylabel('')
plt.title(str(encoder_weights.shape[0]) + ' x ' + str(encoder_weights.shape[1]))
...
assert np.all(decoder_weights == decoder_weights2)

sns.distplot(decoder_weights, hist=True, kde=True, 
             bins=int(180/5), color = 'darkblue', 
             hist_kws={'edgecolor':'black'},
             kde_kws={'linewidth': 4})
# Add labels
plt.suptitle('Dist. decoder weights')
plt.xlabel('Weights')
plt.ylabel('')
plt.title(str(decoder_weights.shape[0]) + ' x ' + str(decoder_weights.shape[1]))
...
dec_weights_dt = pd.DataFrame(decoder_weights)
enc_weights_dt = pd.DataFrame(encoder_weights)
