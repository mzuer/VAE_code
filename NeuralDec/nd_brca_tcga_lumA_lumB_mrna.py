...
# python nd_brca_tcga_lumA_lumB_mrna.py
...
inputfile= os.path.join('/home/marie/Documents/FREITAS_LAB/VAE_tutos/NeuralDec/brca_data/log_filtNormCount_mostVar_dt.txt')

annotfile= os.path.join('/home/marie/Documents/FREITAS_LAB/VAE_tutos/NeuralDec/brca_data/tcga_annot_dt.txt')
...
outfolder = "ND_BREAST_TCGA_LUMA_LUMB_MRNA"
...
lumA_samps = annot_dt[annot_dt['PAM50'] == "lumA"]['cgc_sample_id'].values
lumB_samps = annot_dt[annot_dt['PAM50'] == "lumB"]['cgc_sample_id'].values
...
covar = ['lumA' if x in lumA_samps else 'lumB' for x in df.index]
assert np.sum(np.array(covar) == "lumA") == len(lumA_samps)
assert np.sum(np.array(covar) == "lumB") == len(lumB_samps)
...
min_grid_range_z = 0 # init value -2  # 0-1 because mrna data scaled
max_grid_range_z = 1 # init value -2

min_grid_range_c = 0 # init value -2  # 0-1 because 0 and 1 encoded
max_grid_range_c = 1 # init value -2

covarLab = "lumA-lumB"


outsuffix = "_hd" + str(hidden_dim) + "_nLD" + str(latent_dim) +\
 "_c" + str(covarLab) +  "_" + str(n_iter_integrals) + "_" + str(logging_freq_integrals) +"_"+str(grid_nsteps)+"_"+\
str(min_grid_range_z)+"-"+str(max_grid_range_z)+"_"+str(min_grid_range_c)+"-"+str(max_grid_range_c) 
...
grid_z = torch.linspace(min_grid_range_z, max_grid_range_z, steps=grid_nsteps).reshape(-1, 1).to(device)
grid_c = torch.linspace(min_grid_range_c, max_grid_range_c, steps=grid_nsteps).reshape(-1, 1).to(device)
...
outfile = os.path.join(outfolder, "mu_z" + outsuffix + ".csv")
np.savetxt(outfile, mu_z, delimiter = ',')
print("... written: " + outfile )


outfile = os.path.join(outfolder, "Y_pred_c" + outsuffix + ".csv")
np.savetxt(outfile, Y_pred_c, delimiter = ',')
print("... written: " + outfile )


outfile = os.path.join(outfolder, "Y_pred_cz" + outsuffix + ".csv")
np.savetxt(outfile, Y_pred_cz, delimiter = ',')
print("... written: " + outfile )


outfile = os.path.join(outfolder, "Y_pred_z" + outsuffix + ".csv")
np.savetxt(outfile, Y_pred_z, delimiter = ',')
print("... written: " + outfile )


outfile = os.path.join(outfolder, "Y_pred" + outsuffix + ".csv")
np.savetxt(outfile, Y_pred, delimiter = ',')
print("... written: " + outfile )
